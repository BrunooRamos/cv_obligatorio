#!/usr/bin/env python3
"""
Script para evaluar Liu 2012 en iLIDS-VID con mecanismo de gating adaptativo.

Este script:
1. Extrae features con información de gating de todos los tracklets
2. Evalúa CMC con pooling por tracklet y cross-camera usando gating
3. Muestra los resultados

Ejecutar: python3 eval_ilidsvid_with_gating.py
"""
import numpy as np
...
from eval_ilidsvid import evaluate_cmc_ilidsvid_with_gating
...
def extract_features_ilidsvid_with_gating(sequences_dir, verbose=True, max_frames_per_tracklet=10):
    """
    Extrae features con información de gating de todos los tracklets en iLIDS-VID.
    ...
    """
    print("="*70)
    print("PASO 1: EXTRACCIÓN DE FEATURES CON GATING - iLIDS-VID")
    print("="*70)
        ...
        for idx, person_dir in enumerate(sorted(person_dirs), 1):
            ...
            for frame_file in frame_files:
                ...
                try:
                    ...
                    tracklet_features.append(feature_dict)
                    frames_this_cam += 1
...
def save_features_with_gating(tracklets, output_path):
    """
    Guarda las features con información de gating en disco.
    
    Args:
        tracklets: dict {id: {cam: [feature_dicts...]}}
...
def load_features_with_gating(input_path):
    """
    Carga features con información de gating previamente guardadas desde disco.
    ...
    Returns:
        tracklets: dict {id: {cam: [feature_dicts...]}}
    """
    print(f"Cargando features con gating desde: {input_path}")
...
def evaluate_ilidsvid_with_gating(
    tracklets,
    trials=10,
    pool_method='mean',
    p=None,
    ...
    """
    Evalúa CMC en iLIDS-VID con gating y muestra resultados.
    
    Args:
        tracklets: dict {id: {cam: [feature_dicts...]}}
        trials: número de trials
        pool_method: 'mean' o 'median'
        p: número de IDs a usar por trial (None = min(50, #IDs_disponibles))
    ...
    """
    print("="*70)
    print("PASO 2: EVALUACIÓN CMC CON GATING - iLIDS-VID")
    print("="*70)
    ...
    results = evaluate_cmc_ilidsvid_with_gating(
        tracklets, 
        trials=trials, 
        ranks=(1, 5, 10, 20), 
    ...
    print("\n" + "="*70)
    print("RESULTADOS CMC CON GATING - iLIDS-VID")
    print("="*70)
    print(f"\nConfiguración:")
    print(f"  Trials: {trials}")
    print(f"  Pooling: {pool_method}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluación con gating Liu 2012 en iLIDS-VID')
    ...
    parser.add_argument('--features-file', type=str, default='features_ilidsvid_gating.pkl',
                       help='Archivo para guardar/cargar features (default: features_ilidsvid_gating.pkl)')
    ...
    parser.add_argument('--p', type=int, default=None,
                       help='Número de IDs a usar por trial (default: min(50, #IDs_disponibles))')
    ...
    print("\n" + "="*70)
    print("EVALUACIÓN CON GATING - LIU 2012 EN iLIDS-VID")
    print("="*70)
    if verbose:
    ...
    try:
        ...
        if skip_extraction:
            ...
            if not os.path.exists(features_file):
                ...
                return
            tracklets = load_features_with_gating(features_file)
            total_frames = sum(sum(len(frames) for frames in cams.values()) 
                             for cams in tracklets.values())
        else:
            # Extraer features
            start_time = time.time()
            tracklets, total_frames = extract_features_ilidsvid_with_gating(
                sequences_dir, 
                verbose=verbose, 
            ...
            if not skip_saving:
                save_features_with_gating(tracklets, features_file)
        
        # PASO 2: Evaluar CMC con gating
        results = evaluate_ilidsvid_with_gating(
            tracklets,
            trials=trials,
            pool_method=pool_method,
            p=p,
        ...
        print("✅ Evaluación con gating completada exitosamente!")
        print("\nResultados finales:")
        for rank, accuracy in sorted(results.items()):
